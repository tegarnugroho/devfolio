<template>
  <section v-section-reveal id="skills" class="scroll-mt-12 sm:scroll-mt-16 md:scroll-mt-16 pt-24 md:pt-28 pb-20 border-t border-black/10 dark:border-white/10">
    <div class="mb-8">
      <span v-reveal="{ delay: 0 }" class="text-[11px] tracking-[0.18em] uppercase opacity-60">{{ content.eyebrow }}</span>
      <h2 v-reveal="{ delay: 60, kind: 'heading' }" class="mt-1 text-2xl font-semibold tracking-tight">{{ content.title }}</h2>
      <p v-reveal="{ delay: 120 }" class="mt-3 opacity-75">{{ content.description }}</p>
    </div>
    <ul class="capability-list">
      <li v-for="(row, i) in matrix" :key="row.icon" v-reveal="{ delay: 300 + i * 120, whenVisible: true }" data-blueprint="SKILL_GROUP" class="capability-row">
